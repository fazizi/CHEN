...
export default function ExamScheduler() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("exam-scheduler-data");
        if (result?.value) setExams(JSON.parse(result.value));
      } catch {}
    })();
  }, []);

  // Save to storage
  async function saveExams(data) {
    try {
      await window.storage.set("exam-scheduler-data", JSON.stringify(data));
      setSaved(true);
